...
function App() {
  ...
  return (
    <div className="w-screen h-screen bg-gray-300">
      <div className="absolute top-2 right-2 md:top-5 md:right-5 select-none flex flex-col gap-10">
        {/* Settings */}
        <button onClick={() => setIsSettingOpen(!isSettingOpen)} className="flex justify-center items-center">
      ...
      <div className="absolute md:w-1/4 max-w-fit min-w-fit h-auto p-5 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 select-none">
        ...
        <button onClick={HandleStart} className="w-1/2 text-gray-800 text-xl text-center font-bold px-4 py-2 hover:bg-gray-700 hover:text-white border-l-2 border-r-2 border-gray-800 flex justify-center items-center">
          ...
          <button onClick={() => {
            ...
            // setPomodoros(0);
            setPaused(false);
            const time = modes === "focus" ? convertTimetoTimeStamp(focusTime) : modes === "break" ? convertTimetoTimeStamp(breakTime) : convertTimetoTimeStamp(longBreakTime);
            restart(time, false);
